...
namespace Shutdown
{
    public class Shutdown
    {
        public enum Mode
        {
            ...
            ForcedShutdown = 5,
            Reboot = 2,
            ForcedReboot = 6, // Shutdown 됨
            PowerOff = 8,
            ForcedPowerOff = 12
        }

        private static uint MajorOther = 0x00000000;
        private static uint MinorOther = 0x00000000;
        private static uint Planned = 0x80000000;

        public static void Run(Mode mode, int delaySecond = 0)
        {
            ManagementBaseObject shutdownBaseObject = null;
            ManagementClass win32ManagementClass = new ManagementClass("Win32_OperatingSystem");
            win32ManagementClass.Get();

            win32ManagementClass.Scope.Options.EnablePrivileges = true;
            ManagementBaseObject shutdownParameter = win32ManagementClass.GetMethodParameters("Win32ShutdownTracker");

            shutdownParameter["Flags"] = (int)mode;
            shutdownParameter["ReasonCode"] = MajorOther | MinorOther | Planned;
            shutdownParameter["Timeout"] = delaySecond.ToString();

            foreach (ManagementObject managementObject in win32ManagementClass.GetInstances())
            {
                managementObject.Scope.Options.EnablePrivileges = true;
                shutdownBaseObject = managementObject.InvokeMethod("Win32ShutdownTracker", shutdownParameter, null);
            }
        }
    }
